...
def order_weight(strng):
    stringlist, resultlist, sumlist, twolists = [], [], [], []
    stringdict = {}
    result = ''
    j = 0
    stringlist = strng.strip().split(' ')

    # whitespace and empty member cleaning
    while stringlist:
        try: stringlist.remove(' ')
    ...
    for i in range(len(stringlist)):
        twolists.append(list(str(x[i]) for x in (sumlist, stringlist)))
    twolists.sort(key=lambda tup: tup[1])
    #print(twolists)

    for i in range(len(stringlist)):
        twolists[i][0] = int(twolists[i][0])
    twolists.sort(key=lambda tup: tup[0])
    #print(twolists)

    resultlist = [i[1] for i in twolists]
